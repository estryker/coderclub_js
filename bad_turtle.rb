...
class TurtleBad
  # Opal 0.6.3 is messing this up
  # attr_reader :commands
  
  def initialize
    @commands = []
    `var pen = new Pen("mycanvas");`
  end
  ...
  def add_command(command)
    # Opal seems to have trouble with the initializing of @commands ...
    if(@commands.nil?)
      @commands = [command]
    else
      @commands << command
  ...
  def width
    %x{
      console.warn('width not implemented yet')
      #{color}
       }
    return nil
  end
