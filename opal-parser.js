...
(function($opal) {
  ...
  return (function($base) {
    ...
    $opal.cdecl($scope, 'VERSION', "0.6.3")
...
(function($opal) {
  ...
  return (function($base) {
    ...
    $opal.defs(self, '$compile', function(source, options) {
      ...
      // adding the compile_to_sexp capability (ethan)
      $opal.defs(self, '$compile_to_sexp', function(source, options) {
      var self = this;

      if (options == null) {
        options = $hash2([], {})
      }
      return $scope.Compiler.$new().$compile_to_sexp(source, options);
    });
    ...
    (function($base, $super) {
      ...
      // TODO: can I add a compile option to have it spit out sexps ??? (ethan)
      ...
      // added this function to stop short in the compilation process and just spit out the sexp (ethan)
      def.$compile_to_sexp = function(source, options) {
        var $a, $b, self = this;
        ...
        self.parser = $scope.Parser.$new();
        self.sexp = self.$s("top", ((($a = self.parser.$parse(self.source, self.$file())) !== false && $a !== nil) ? $a : self.$s("nil")));
        console.warn("in compile_to_sexp!")
	// stop short here, and return the Sexp object (ethan)
        return self.sexp

	// commenting out the rest (ethan) 
        //self.eof_content = self.parser.$lexer().$eof_content();
        //self.fragments = self.$process(self.sexp).$flatten();
        //return self.result = ($a = ($b = self.fragments).$map, $a._p = "code".$to_proc(), $a).call($b).$join("");
      };
...
(function($opal) {
  ...
  // added by (ethan)
  Opal.compile_to_sexp = function(str, options) {
    if (options) {
      options = Opal.hash(options);
    }
    return Opal.Opal.$compile_to_sexp(str, options);
  };

  // not working (ethan)
  Opal.parse_to_sexp = function(str, options) {
    if (options) {
      options = Opal.hash(options);
    }
    return Opal.top.$parse_to_sexp(str, options);
  };
  // not working  (ethan)
  Opal.parser = function(str, options) {
    if (options) {
      options = Opal.hash(options);
    }
    return Opal.top.$parser(str, options);
  };
